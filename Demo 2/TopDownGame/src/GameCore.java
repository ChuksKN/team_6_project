...
public class GameCore extends Canvas implements Runnable {
	...
	public GameCore() {
		...
		level = imgLoader.loadImage("/dem2_testbackdropver3.png");
		sprite_sheet = imgLoader.loadImage("/PS_Down-1.png");
	...
	private void loadLevel(BufferedImage image) {
		...
		for(int xx = 0; xx < width; xx++) {
			for(int yy = 0; yy < height; yy++) {
				...
				if (blue == 255)
					objHandler.addObject(new Player(xx*32, yy*48, ID.Player, objHandler, newSprite));
